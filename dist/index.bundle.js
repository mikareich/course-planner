(function () {
  ...
  function eventHandler(event) {
    ...
    const course = element.id; // cancel if disabled
    ...
    const possibilities = findCoursePossibilties(selectedCourses); // disabled other items
  ...
  function findCoursePossibilties(selectedCourses) {
    ...
    const selectedCategories = selectedCourses.map(course => getCategory(course)); // search for possibilities

    const possibleCategoryCombinatinons = courseCombinations.map(courseCombination => {
      // get categories of maped combi
      const combinationCategory = courseCombination.map(combi => getCategory(combi[0])); // check if combi matches
      ...
      if (matched) return combinationCategory.map(category => categories[category]).flat();
    });
    return [...new Set(possibleCategoryCombinatinons.flat().filter(n => n))];
  }

  function getCategory(course) {
    return Object.entries(categories).filter(([category, courses]) => courses.includes(course))[0][0];
  }
